...
from conversors import from_system_time_to_gmt, str_to_datetime
...
def get_user_activity_by_widget_type(
    client: Client, board_id: str, from_minutes: int, type: str = "sticker"
):
    ...
    for item in items:
        ...
        filtered_dict = filter_keys(
            item_with_datetime, ["modifiedAt", "modifiedBy", "type"]
        )
    ...
    time_gmt_minus_n_minutes = from_system_time_to_gmt(datetime.now()) - timedelta(
        minutes=from_minutes
    )

    board_data = list(
        filter(lambda x: x["modifiedAt"] >= time_gmt_minus_n_minutes, board_data)
    )
...
def get_user_activity(client: Client, board_id: str, from_minutes: int) -> Dict:
    ...
    for item in board["data"]:
        item_with_datetime = str_to_datetime(item, ["modifiedAt"])
        filtered_dict = filter_keys(item_with_datetime, ["modifiedAt", "modifiedBy"])
...
def get_user_data_with_text_fields(
    client: Client, board_id: str, from_minutes: int
) -> Dict:
    board = client.list_all_widgets(board_id)
    ...
    for item in board["data"]:
        if item["type"] in types:
            ...
            filtered_dict = filter_keys(
                item_with_datetime, ["modifiedAt", "modifiedBy", "text"]
            )
...
def get_user_reports(user_activity):
    sorted_user_activity = sorted(
        user_activity, key=lambda x: x["modifiedAt"], reverse=False
    )
