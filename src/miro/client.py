...
class Client(object):
    def __init__(self, base_url: str, auth_token: str) -> None:
        ...
        self.auth_header = {"Authorization": f"Bearer {self._auth_token}"}

    def get_current_token_context(self):
    ...
    def get_board_members(self, board_id: str, flat: bool = False) -> Dict:
        url = f"{self.base_url}/v1/boards/{board_id}/user-connections"
        response = httpx.get(url, headers=self.auth_header)
        data = handle_json(response)
        return ListObject(data, flat=flat)
    ...
    def get_logs(
        ...
        end_date="2021-31-02T05:34:08.000Z",
        limit="10",
        offset="0",
    ) -> Dict:
        ...
        response = httpx.get(url, params=querystring, headers=self.auth_header)
        return handle_json(response)
    ...
    def get_team_boards(self, team_id) -> Dict:
        url = f"{self.base_url}/v1/teams/{team_id}/boards"
        response = httpx.get(url, headers=self.auth_header)
        return handle_json(response)

    def get_team_user_connections(self, team_id):
        url = f"{self.base_url}/v1/team-user-connections/{team_id}"
        response = httpx.get(url, headers=self.auth_header)
        return handle_json(response)
    ...
    def list_all_team_members(self, team_id):
        url = f"{self.base_url}/v1/teams/{team_id}/user-connections"
        response = httpx.get(url, headers=self.auth_header)
        data = handle_json(response)
        return ListObject(data)

    def list_all_widgets(self, board_id):
